...
export function attachCctvVideo(video, url, feedType, {
  ...
  const mediaUrl = feedType === 'hls' ? `${url}${url.includes('?') ? '&' : '?'}lease=${leaseId}` : url;
  let timer = null;
  let startup = null;
  let governor = null;
  let retries = 0;
  ...
  const ready = (async () => {
    if (feedType !== 'hls') { video.src = mediaUrl; return; }
    try {
      ...
      hls = new Hls({ enableWorker: true, maxBufferLength: 24, maxMaxBufferLength: 30, backBufferLength: 0, maxBufferSize: 16 * 1024 * 1024, liveSyncDurationCount: 3 });
      ...
      governor = setInterval(() => {
        ...
        video.playbackRate = ahead < 6 ? 0.8 : ahead < 12 ? 0.9 : ahead > 24 ? 1.05 : 1;
      }, 1000);
