  ...
  const layer = {
    ...
    getRowControls() {
      ...
      return {
        chips: ['gfs', 'ifs'].map((value) => ({
          ...
          title:
            value === 'ifs'
              ? 'ECMWF IFS 10 m forecast'
              : 'NOAA GFS 10 m forecast',
        })),
        legend: [
        ...
        info: `${model === 'ifs' ? 'ECMWF IFS' : 'GFS'} forecast · Valid: ${valid || 'Unavailable'} · Issued: ${run || 'Unavailable'}${manifest?.stale ? ' · STALE' : ''}`,
      };
    },
    ...
    getStats() {
      return {
        ...
        error: error || windStats(manifest).error,
      };
    },
    getParticleCount() {
