...
export async function fetchHlsBytes(url, { fetchImpl = fetch, signal, maxBytes, timeoutMs = HLS_LIMITS.timeoutMs } = {}) {
  ...
  try {
    ...
    if (!response.ok || response.redirected || (response.url && new URL(response.url).href !== new URL(url).href))
      throw new Error('HLS upstream refused');
    if (Number(response.headers.get('content-length')) > maxBytes) throw new Error('HLS response too large');
...
export function sameOriginHlsUrl(value, base) {
  ...
  if (!['https:', 'http:'].includes(url.protocol) || url.origin !== new URL(base).origin || url.username || url.password)
...
export function parseHlsMedia(text, base, limit = HLS_LIMITS.segments) {
  if (!text.startsWith('#EXTM3U') || /#EXT-X-(?:KEY|MAP|BYTERANGE|I-FRAMES-ONLY)/.test(text))
  ...
  for (const raw of text.split('\n')) {
    ...
    if (line.startsWith('#EXT-X-MEDIA-SEQUENCE:')) {
      ...
      if (!Number.isSafeInteger(seq) || seq < 0) throw new Error('Invalid media sequence');
    } else if (line === '#EXT-X-DISCONTINUITY') {
      discontinuity = true;
    } else if (line.startsWith('#EXTINF:')) {
      duration = Number.parseFloat(line.slice(8));
      if (!Number.isFinite(duration) || duration <= 0 || duration > 60) throw new Error('Invalid segment duration');
    } else if (line && !line.startsWith('#')) {
      if (duration === null || !Number.isSafeInteger(seq)) throw new Error('Invalid HLS segment');
      const uri = sameOriginHlsUrl(line, base);
      if (!new URL(uri).pathname.endsWith('.ts')) throw new Error('Only MPEG-TS segments are supported');
...
export function createHlsPuller({ fetchImpl = fetch, limits = HLS_LIMITS } = {}) {
  ...
  const touch = (entry, leaseId, create = false) => {
    ...
    if (!lease) {
      if (!create || entry.leases.size >= limits.leasesPerSession) throw new Error('HLS lease capacity reached');
      lease = {};
      entry.leases.set(leaseId, lease);
    }
    clearTimeout(lease.timer);
    lease.timer = setTimeout(() => release(entry.cameraId, leaseId), limits.idleMs);
    lease.timer.unref?.();
  };
  const read = (entry, url, maxBytes) => fetchHlsBytes(url, {
    fetchImpl, signal: entry.controller.signal, maxBytes, timeoutMs: limits.timeoutMs,
  });
  const poll = async (entry) => {
    try {
      let base = entry.chunklistUrl || entry.url;
      let text = (await read(entry, base, limits.playlistBytes)).toString('utf8');
      if (!text.includes('#EXTINF:')) {
        const variant = text.split('\n').map((line) => line.trim()).find((line) => line && !line.startsWith('#'));
        if (!variant || !text.includes('#EXT-X-STREAM-INF:')) throw new Error('Missing HLS variant');
      ...
      const restarted = newest < entry.upstreamNewest || segments.some((segment) => {
      ...
      for (const segment of segments) {
        ...
        while (entry.segments.size && (entry.bytes + body.length > limits.sessionBytes || entry.segments.size >= limits.segments)) {
        ...
        if (body.length > limits.sessionBytes) throw new Error('Segment exceeds session budget');
        const seq = entry.nextSeq++;
        entry.segments.set(seq, { ...segment, seq, body, discontinuity: segment.discontinuity || entry.pendingDiscontinuity });
        entry.pendingDiscontinuity = false;
        entry.upstream.set(segment.seq, segment.uri);
        while (entry.upstream.size > limits.segments * 2) entry.upstream.delete(entry.upstream.keys().next().value);
      ...
      entry.failures++;
      if (entry.failures >= 3) { stop(entry.cameraId, entry.token); return; }
    } finally {
      if (!entry.stopping) {
        entry.timer = setTimeout(() => { entry.polling = poll(entry); }, limits.pollMs);
        entry.timer.unref?.();
      }
    }
  };
  const ensure = async (cameraId, url, leaseId = 'legacy') => {
    ...
    if (entry && entry.url !== url) { stop(cameraId); entry = null; }
    if (entry) { touch(entry, leaseId, true); return entry; }
    // Reserve synchronously, before any await: concurrent creation is singleflight.
    if (active.size >= limits.sessions) throw new Error('HLS session capacity reached');
    entry = { cameraId, url, token: randomUUID(), controller: new AbortController(), segments: new Map(), bytes: 0, stopping: false, failures: 0, nextSeq: 0, upstream: new Map(), leases: new Map(), upstreamNewest: -1, pendingDiscontinuity: false, discontinuitiesRemoved: 0 };
  ...
  const buildPlaylist = async (entry, cameraId, leaseId = 'legacy') => {
    ...
    const lines = ['#EXTM3U', '#EXT-X-VERSION:3', `#EXT-X-TARGETDURATION:${Math.ceil(Math.max(...segments.map((s) => s.duration)))}`, `#EXT-X-MEDIA-SEQUENCE:${segments[0].seq}`, `#EXT-X-DISCONTINUITY-SEQUENCE:${entry.discontinuitiesRemoved}`];
    let previous;
    for (const segment of segments) {
      if (segment.discontinuity || (previous !== undefined && segment.seq !== previous + 1)) lines.push('#EXT-X-DISCONTINUITY');
      lines.push(`#EXTINF:${segment.duration.toFixed(3)},`, `/api/cctv/media/${encodeURIComponent(cameraId)}/seg_${segment.seq}.ts?session=${entry.token}&lease=${encodeURIComponent(leaseId)}`);
  ...
  const getSegment = (cameraId, token, seq, leaseId = 'legacy') => {
    const entry = active.get(cameraId);
    if (!entry || entry.token !== token || !entry.leases.has(leaseId)) return null;
  ...
  return { ensure, waitReady, buildPlaylist, getSegment, stop, release, shutdown,
    stats: () => ({ sessions: active.size, bytes: [...active.values()].reduce((total, e) => total + e.bytes, 0) }) };
}
