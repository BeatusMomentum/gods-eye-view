...
function loadEccodes() {
  eccodesPromise ??= import('@meri-imperiumi/eccodes-wasm').then(module => module.createEccodes()).catch(error => {
    eccodesPromise = null;
    throw error;
  });
...
export async function decodeWindGribMessage(buffer, { eccodesModule } = {}) {
  if (buffer.byteLength > 8 * 1024 * 1024) throw new Error('Wind GRIB exceeds byte budget');
  ...
  try {
    ...
    if (!Number.isInteger(ni) || !Number.isInteger(nj) || ni < 1 || nj < 1 || ni * nj > 2_000_000) throw new Error('Invalid wind GRIB geometry');
    return {
      ni, nj,
